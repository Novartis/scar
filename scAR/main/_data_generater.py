...
class CITEseq(scRNAseq_synthetic):
    ...
    def generate(self):
        ...
        native_signals = np.vstack(
            [random.multinomial(n=tot_c, pvals=theta1) for tot_c, theta1 in zip(tot_count1, beta_in_each_cell)]
        )
    ...
    def heatmap(self, return_obj=False):
        ...
        sns.heatmap(np.log2(obs + 1), yticklabels=False, vmin=0, vmax=10, cmap="coolwarm", center=2, ax=axs[0])
        axs[0].set_title("observation")

        sns.heatmap(np.log2(ambient_signals + 1), yticklabels=False, vmin=0, vmax=10, cmap="coolwarm", center=2, ax=axs[1])
        axs[1].set_title("ambient signals")

        sns.heatmap(np.log2(native_signals + 1), yticklabels=False, vmin=0, vmax=10, cmap="coolwarm", center=2, ax=axs[2])
...
class CROPseq(scRNAseq_synthetic):
    ...
    def heatmap(self, return_obj=False):
        ...
        fig, axs = plt.subplots(ncols=3, figsize=(15, 5))
        sns.heatmap(np.log2(obs + 1), yticklabels=False, vmin=0, vmax=7, cmap="coolwarm", center=2, ax=axs[0])
        axs[0].set_title("observation")

        sns.heatmap(np.log2(ambient_signals + 1), yticklabels=False, vmin=0, vmax=7, cmap="coolwarm", center=2, ax=axs[1])
        axs[1].set_title("ambient signals")

        sns.heatmap(np.log2(native_signals + 1), yticklabels=False, vmin=0, vmax=7, cmap="coolwarm", center=2, ax=axs[2])
        axs[2].set_title("native signals")
